...
counter = 0
auxOutputFolder = [f.name for f in os.scandir() if f.is_file()]
if "outAux.txt" in auxOutputFolder:
        subprocess.run(["rm", "outAux.txt"])
for request in requestList:
    if "request.json" in auxOutputFolder:
        subprocess.run(["rm", "request.json"])
    print("generating request file")
    counter = counter +1
    finalJson = []
    row = "{"
    rowa = " \"path-request\": ["
    ...
    with open("request.json", "w") as f:
        for row in finalJson:
            ...
            f.write(line)
        f.close() 
    print("Request " + str(counter) + " created")
    print("Calling gnpy")
    subprocess.run(["gnpy-path-request", "-o", "requestOut.json", "network.json", "request.json", "-e", "equipments.json"])
    print("Reading gnpy output and appending to results file")
    subprocess.run(["python3","requestOutputReader_vCompare.py"])
    print("finishing request " + str(counter) )