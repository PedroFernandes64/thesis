...
class MainWindow(tk.Tk):
    ...
    def create_param(self):
        ...
        if self.topologyFile.get() != '':
            ...
            os.system(self.topologyFile.get() + " > topologyFile.txt")
    ...
    def run(self):
        if self.paramFileFolder.get() != '':
            ...
            os.system(self.paramFileFolder.get() + " > paramFolderPath.txt")
            file = open('paramFolderPath.txt', 'r').readline()
            paramFilePath.set(file.strip('\n'))
            print("./exec " + paramFilePath.get() + " > output.txt")
            os.system("./exec " + paramFilePath.get() + " > output.txt")
            self.text_box.delete(1.0, 'end')
            self.text_box.insert("end", self.read_exec())
        else:
            os.system("./exec onlineParameters.txt > output.txt")
            self.text_box.delete(1.0, 'end')
            self.text_box.insert("end", self.read_exec())
